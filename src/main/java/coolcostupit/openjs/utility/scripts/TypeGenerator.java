...
public class TypeGenerator {
    ...
    private String generateDeclaration(Class<?> clazz) {
        ...
        // Use full dotted name as the TS identifier to avoid collisions
        String tsName = "_" + sanitizeName(clazz.getName());

        StringBuilder sb = new StringBuilder();
        sb.append("/** `importClass(\"").append(clazz.getName()).append("\")` */\n");

        if (clazz.isEnum()) {
            sb.append("declare class ").append(tsName).append(" {\n");
            try {
                for (Object c : clazz.getEnumConstants()) {
                    sb.append("  static readonly ").append(sanitizeName(c.toString()))
                            .append(": ").append(tsName).append(";\n");
                }
            } catch (Throwable ignored) {}
            sb.append("  name(): string;\n");
            sb.append("  toString(): string;\n");
            sb.append("  static valueOf(name: string): ").append(tsName).append(";\n");
            sb.append("  static values(): ").append(tsName).append("[];\n");
        } else if (clazz.isInterface()) {
            sb.append("declare interface ").append(tsName).append(" {\n");
            appendMethods(sb, clazz, false, tsName);
        } else {
            sb.append("declare class ").append(tsName).append(" {\n");
            appendConstructors(sb, clazz, tsName);
            appendFields(sb, clazz);
            appendMethods(sb, clazz, true, tsName);
        }
    ...
    private String typeToTs(Class<?> t) {
        ...
        // Use the collision-free prefixed name
        return "_" + sanitizeName(t.getName());
    }
    ...
    private void writeImportClassOverloads() throws IOException {
        // Collect all class names from all generated files
        ...
        for (String name : classNames) {
            String tsName = "_" + name.replaceAll("[^a-zA-Z0-9_]", "_");
